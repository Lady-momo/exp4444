...
def evaluate(net, dataloader, device, amp):
    ...
    with torch.autocast(device.type if device.type != 'mps' else 'cpu', enabled=amp):
        for batch in tqdm(dataloader, total=num_val_batches, desc='Validation round', unit='batch', leave=False):
            ...
            cross_entropy += F.cross_entropy(net(image), batch['mask'].to(device)).item()
...
def dice_coeff(input: Tensor, target: Tensor):
    ...
    sum_input = input.sum(dim=(2, 3))
    sum_target = target.sum(dim=(2, 3))
    dice = (2. * intersection + smooth) / (sum_input + sum_target + smooth)
...
def mIoU(input: Tensor, target: Tensor):
    ...
    sum_input = input.sum(dim=(2, 3))
    sum_target = target.sum(dim=(2, 3))
    union = sum_input + sum_target - intersection
